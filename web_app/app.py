...
with st.sidebar:
    selected = option_menu(
        menu_title="Explore",
        options=["Introduction", "EDA", "Visualizations", "Consumer Price Index", "GDP","Notebook Viewer","Contact"],
        menu_icon="cast"
    )

if selected == "Introduction":
    st.title('Naira Metrics: Price Surge in Nigerian Economy')
...
if selected == "Visualizations":
    ...
    ax.plot(years, gdp_constant, label='GDP (constant LCU)', color='green', marker='o')
    ax.set_xlabel('Year')
    ...
    fig, ax = plt.subplots(figsize=(10, 6))
    sns.lineplot(x=years, y=gdp_growth, marker='o', color='green', linewidth=2.5, ax=ax)
    ...
    fig, ax = plt.subplots(figsize=(10, 6))
    ax.plot(years, cpi, label='CPI (2010=100)', marker='o', color='blue')
    ax.set_xlabel('Year')
    ax.set_ylabel('CPI')
    ...
    fig, ax = plt.subplots(figsize=(10, 6))
    sns.lineplot(x=years, y=cpi, marker='o', label='Consumer price index (2010 = 100)', linewidth=2.5, ax=ax)
    sns.lineplot(x=years, y=inflation, marker='o', label='Inflation, consumer prices (annual %)', linewidth=2.5, ax=ax, color='red')
    ...
    fig, ax = plt.subplots(figsize=(10, 6))
    sns.lineplot(x=years, y=gdp_growth, marker='o', label='GDP Growth (annual %)', color='green', ax=ax)
    sns.lineplot(x=years, y=inflation, marker='o', label='Inflation (annual %)', color='red', ax=ax)
    sns.lineplot(x=years, y=crude_oil_price, marker='o', label='Crude Oil Price ($)', color='orange', ax=ax)
    ax.set_xlabel('Year')
    ...
    fig, ax = plt.subplots(figsize=(10, 6))
    sns.lineplot(x=years, y=trade_openness, marker='o', label='Trade Openness Index (%)', ax=ax)
    sns.lineplot(x=years, y=crude_oil_price, marker='o', label='Crude Oil Price (per barrel in $)', ax=ax)
    ...
    ax_petrol.plot(years, data['Petrol Price (Naira)'], marker='o')
    ...
    ax.bar(years, data['Population, total'], color='skyblue')
    ax.set_xlabel('Year')
    ...
    fig, ax = plt.subplots(figsize=(10, 6))
    if 'Regime Type' in data.columns:
        ...
        st.write("The 'Regime Type' column is not available in the data.")

    st.markdown('<h4><u>Monetary Policy & Lending Interest</u></h4>', unsafe_allow_html=True)
    
    # Plotting Line plots of Monetary Indicators      
    st.write('***Monetary Indicators Over Time***')
    fig, ax = plt.subplots(figsize=(10, 6))
    sns.lineplot(x=years, y=data['Money Supply M3'], marker='o', label='Money Supply M3', ax=ax)
    sns.lineplot(x=years, y=data['Base Money'], marker='o', label='Base Money', ax=ax)
    sns.lineplot(x=years, y=data['Currency in Circulation'], marker='o', label='Currency in Circulation', ax=ax)
    ax.set_xlabel('Year')
    ...
    st.pyplot(fig_debt)
    st.write("""
    This chart shows the trend of Debt % GDP over the years. From 1990 to 2023, Nigeria's debt percentage has fluctuated. It was high in the early 1990s, declined until 1999, then peaked in the late 1990s. From 2006 to 2010, debt percentage significantly reduced, likely due to better economic management or debt relief. However, since 2011, the debt percentage has been rising, raising concerns about the sustainability of Nigeria's fiscal policies and economic health.
    """)
...
# Consumer Price Index
    ...
    - **Inflation Indicator:** CPI is a crucial indicator for measuring inflation and deflation.
    - **Economic Policy:** It aids policymakers and economists in making economic decisions.
    - **Income Planning:** Adjusts income payments, reflecting changes in cost of living.
    
    ## How Does This Tool Help?
    - **Prediction:** Based on historical data and economic indicators, this tool predicts future CPI values, enabling users to anticipate changes in living costs.
    - **Data Driven:** Utilizes machine learning to analyze trends and make predictions.
    """)
...
#----------------------------------------------GDP-------------------------------------------------------------------
    ...
    Gross Domestic Product (GDP) per capita divides the GDP by the population size, indicating the economic output per person.

    ## Why is GDP per Capita Important?
    - **Standard of Living:** It is often considered an indicator of a country's standard of living.
    - **Economic Health:** Reflects the economic health of a country and is used to compare the prosperity of different countries.
    
    ## How Does This Tool Help?
    - **Economic Forecasting:** Helps in forecasting the economic growth and providing a snapshot of the economic health.
    - **Policy Making:** Aids in policy making by providing insights into the economic impact of different policy decisions.
    - **Investment Decisions:** Investors use GDP per capita for making informed investment decisions.
    """)
    ...
    st.title("Omdena")
    st.write("""This project was created in collaboration with Omdena Kaduna Chapter, where I worked as a Lead Machine Learning Engineer. 
             Omdena is a global platform that collaborates on real-world problems through AI innovation, bringing together engineers, researchers, and domain experts.""")
    st.write("""
